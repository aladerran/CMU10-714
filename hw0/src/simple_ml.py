...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(num_batches):
        ...
        theta_grad = (X_batch.T @ (Z - I_y)) / batch
        theta -= lr * theta_grad
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(num_batches):
        ...
        Z2 = np.exp(Z1 @ W2)
        Z2 = Z2 / np.sum(Z2, axis=1)[:, None]
        I_y = np.zeros_like(Z2)
        I_y[np.arange(len(y_batch)), y_batch] = 1
        W2_grad = (Z1.T @ (Z2 - I_y)) / batch
        W1_grad = (X_batch.T @ ((Z2 - I_y) @ W2.T * (Z1 > 0))) / batch
        W1 -= lr * W1_grad
        W2 -= lr * W2_grad
    ### END YOUR CODE
